...
cnx = connection.MySQLConnection(user='old', password='old',
                                 host='127.0.0.1', port='6603',
                                 database='old-questions')
...
def anonynisation(fixedvalue_newpasswd):
    ...
    for (first_name, last_name, username, email, normalized_username, password) in cursor1:
        ...
        cursor2.execute(
            "UPDATE user SET first_name = '{}', last_name = '{}', email = '{}', password = '{}' WHERE username = '{}'".format(
                new_first_name, new_last_name, new_email, new_password, username))
